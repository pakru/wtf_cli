...
func TestChatHandler_buildChatMessages_Empty(t *testing.T) {
	...
	// Should have a single system message (prompt + TTY context combined)
	if len(messages) < 1 {
		t.Fatalf("Expected at least 1 message, got %d", len(messages))
	}
...
func TestChatHandler_buildChatMessages_FiltersThinkingPlaceholder(t *testing.T) {
	...
	// system + user + real assistant
	if len(messages) != 3 {
		t.Fatalf("Expected 3 messages, got %d", len(messages))
	}
	if messages[1].Role != "user" || messages[1].Content != "Hello" {
		t.Fatalf("Expected preserved user message, got role=%q content=%q", messages[1].Role, messages[1].Content)
	}
	if messages[2].Role != "assistant" || messages[2].Content != "Real answer" {
		t.Fatalf("Expected preserved assistant message, got role=%q content=%q", messages[2].Role, messages[2].Content)
	}
}
...
func TestChatHandler_MessageCapping(t *testing.T) {
	...
	// buildChatMessages adds system + all input history
	// So: 1 (system) + 15 (all input history) = 16
	expectedCount := 1 + len(history) // 1 + 15 = 16
...
func TestChatHandler_MessageCapping_ExactLimit(t *testing.T) {
	...
	// Should have system + MaxChatHistoryMessages
	expectedCount := 1 + MaxChatHistoryMessages
	if len(messages) != expectedCount {
		t.Errorf("Expected %d messages, got %d", expectedCount, len(messages))
	}
}
...
func TestChatHandler_ContextBuilding(t *testing.T) {
	...
	// Should have a system message with TTY context embedded
	if len(messages) < 1 {
		t.Fatalf("Expected at least 1 message, got %d", len(messages))
	}
	...
	if systemMsg.Role != "system" {
		t.Errorf("Expected system role, got %q", systemMsg.Role)
	}

	// Should have some content (terminal output context combined into system prompt)
	if len(systemMsg.Content) == 0 {
		t.Error("Expected system message to have TTY context content")
	}
}
...
func TestChatHandler_buildChatMessages_UsesNonDiagnosticPrompt(t *testing.T) {
	...
	// Combined system message must NOT contain /explain diagnostic wording
	...
	// Combined system message must NOT contain /explain user prompt wording
	if strings.Contains(systemMsg.Content, "explain what's going on") {
		t.Errorf("Chat system message must not contain 'explain what's going on', got: %q", systemMsg.Content)
	}

	// Combined system message must still have <cmd> instruction
